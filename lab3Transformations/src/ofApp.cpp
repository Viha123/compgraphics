...
bool Plane::intersect(const Ray &ray, glm::vec3 &point,
  ...
  if (hit) {
    ...
    glm::vec2 xrange =
        glm::vec2(position.x - width / 2, position.x + width / 2);
    glm::vec2 yrange =
        glm::vec2(position.y - width / 2, position.y + width / 2);
    glm::vec2 zrange =
        glm::vec2(position.z - height / 2, position.z + height / 2);
    ...
    //
      ...
      if (point.x < xrange[1] && point.x > xrange[0] && point.y < yrange[1] &&
          point.y > yrange[0]) {
        insidePlane = true;
      }
    }
    ...
    //
      ...
      if (point.y < yrange[1] && point.y > yrange[0] && point.z < zrange[1] &&
          point.z > zrange[0]) {
        insidePlane = true;
      }
    }
...
void Pyramid::draw() {
  ...
  for (int i = 0; i < tri.size(); i++) {
    ...
    //
    glm::vec3 normal =
        glm::cross(glm::normalize(v3 - v2), glm::normalize(v1 - v2));
...
void ofApp::setup() {
  ...
  //
  scene.push_back(
      new Plane(glm::vec3(0, -1, 0), glm::vec3(0, 1, 0), ofColor::darkGreen));
...
void ofApp::update() {
  if (objSelected()) {
    selected[0]->rotation.x += 1.0;
    selected[0]->rotation.y += 1.0;
    selected[0]->rotation.z += 1.0;
  }
}
...
void ofApp::keyPressed(int key) {
  ...
  case 'n': {
    if (objSelected()) {
      selected[0]->scale[0] *= 2;
      selected[0]->scale[1] *= 1.5;
      selected[0]->scale[2] *= 0.5;
		}
		break;
	}
  case 'x':
...
void ofApp::removeObject(SceneObject *obj) {
  ...
  // delete object;  sceneObject destructor will handle cleaning up parent and
  // child lists.
  //
  delete obj;
}
