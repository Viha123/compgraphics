...
public:
  ...
  glm::mat4 getMatrix() {
    ...
    glm::mat4 R =
        glm::eulerAngleYXZ(glm::radians(rotation.y), glm::radians(rotation.x),
                           glm::radians(rotation.z)); // yaw, pitch, roll
    glm::mat4 S = glm::scale(
        glm::mat4(1.0),
        scale); // added this scale if you want to change size of object
    return T * R * S;
  }
  ...
  void resetTransform() { transform = getMatrix(); }

  // material properties (we will ultimately replace this with a Material class
  // - TBD)
  //
...
public:
  Plane(glm::vec3 p, glm::vec3 n, ofColor diffuse = ofColor::green,
...
public:
  ...
  Pyramid(glm::vec3 tran, glm::vec3 rot, glm::vec3 sc,
...
public:
  Box(ofColor color = ofColor::red) {
    init();
    diffuseColor = color;
  }
  Box(glm::vec3 tran, glm::vec3 rot, glm::vec3 sc,
          ofColor color = ofColor::blue) {
    init();
    position = tran;
    rotation = rot;
